...
$text = "SELECT * FROM conff where conff_id IN
			(SELECT conff_id
					FROM conff_tag
					INNER JOIN user_tag ON conff_tag.tag_id = user_tag.tag_id
					WHERE user_tag.user_id = $user_id
				UNION 
			SELECT conff_id
					FROM conff_like
					INNER JOIN user_friend ON conff_like.user_id = user_friend.friend_id
					WHERE user_friend.user_id = $user_id
				UNION 
			SELECT conff_id
					FROM comment 
					INNER JOIN user_friend ON comment.user_id = user_friend.friend_id
					WHERE user_friend.user_id = $user_id)";
