...
$liked_comment = array();
////////////////////////////////////////////////tags///////////////////////////////////////////////////
$tags = mysql_query("SELECT * FROM user_tag where user_id = ".$user_id." ;");
		
		while($row = mysql_fetch_array($tags))
		{
			
			$tag_conff = mysql_query("SELECT * FROM conff_tag where tag_id = ".$row['tag_id']." limit $all ;");
			//many conff_id
			while($row = mysql_fetch_array($tag_conff))
			{
...
{	
	$friends = $friends.' or user_id = '.$row['friend_id'];
}

$query_comment = mysql_query("SELECT * FROM comment where $friends limit $all ;");
		while($row = mysql_fetch_array($query_comment))
		{
				array_push($conff, $row['conff_id']);
				$comments = array();
				$comments['conff_id'] = $row['conff_id'];
				$comments['friend_id'] = $row['user_id'];
				array_push($commented, $comments);
		}

		//print_r($commented);
		$like_conff = mysql_query("SELECT * FROM conff_like where $friends limit $all ;");
		//many conff_id
		while($row = mysql_fetch_array($like_conff))
			{
				array_push($conff, $row['conff_id']);
				$likes = array();
				$likes['conff_id'] = $row['conff_id'];
				$likes['friend_id'] = $row['user_id'];
				array_push($liked_arr, $likes);
			}
		//print_r($liked_arr);

		// $query_comment_like = mysql_query("SELECT * FROM comment_like where $friends limit $all ;");
		// while($row = mysql_fetch_array($query_comment_like))
		// {
			// $comment_like_conff = mysql_query("SELECT * FROM comment where comment_id = ".$row['comment_id']." ;");
			//many conff_id
			// while($row = mysql_fetch_array($comment_like_conff))
			// {
				// array_push($conff, $row['conff_id']);
				// array_push($liked_comment, $row['conff_id']);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
					...
					for($i=1;$i<sizeof($conff);$i++)
					{
						...
						//{
						$get_conff = mysql_query("SELECT * FROM conff where $orconff order by activity_time desc limit $all;");
					...
					//print_r(mysql_num_rows($get_conff));
						while($row = mysql_fetch_array($get_conff))
						{
							if( $time - $row['activity_time'] < 50 || $_GET['first']==1){
								...
								//print_r($commented);
								// if(in_array($conff_id, $liked_comment))
									 // {
										// $get_comment_id = mysql_query("SELECT comment_id FROM comment where conff_id = ".$conff_id." ORDER BY comment_id DESC LIMIT 1 ;");
										// if (mysql_num_rows($get_comment_id) > 0) 
										// {
											// $row1 = mysql_fetch_row($get_comment_id);
											// $get_person_id = mysql_query("SELECT user_id FROM comment_like where comment_id = ".$row1[0]." ORDER BY id DESC LIMIT 1 ;");
											// $person_id = mysql_fetch_row($get_person_id);
											// $id =  $person_id[0];
											// $get_person_name = mysql_query("SELECT name FROM users where user_id = '$id' ;");
											// $person = mysql_fetch_row($get_person_name);
											// $conff["special_name"] = $person[0];
											// $conff["special_id"] = $person_id[0];
											// $conff["special_type"] = 3;			//1 for comment 0 for like
										// }
									// }
								
								if(in_multiarray($row['conff_id'], $commented,'conff_id'))
									{
											$use_id = search_multiarray($row['conff_id'], $liked_arr,'conff_id');
											$get_person_name = mysql_query("SELECT name FROM users where user_id = '$use_id' ;");
											$person = mysql_fetch_row($get_person_name);
											$conff["special_name"] = $person[0];
											$conff["special_id"] = $use_id;
											$conff["special_type"] = 1;
									}
								
								else if(in_multiarray($row['conff_id'], $liked_arr,'conff_id'))
									{
											$use_id = search_multiarray($row['conff_id'], $liked_arr,'conff_id');
											$get_person_name = mysql_query("SELECT name FROM users where user_id = '$use_id' ;");
											$person = mysql_fetch_row($get_person_name);
