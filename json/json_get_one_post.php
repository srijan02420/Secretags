...
$time = time();
		$get_conff = mysql_query("SELECT * FROM conff where conff_id = $conff_id ;");
    ...
    // no products found
    $response["success"] = 0;
	$lastconff = mysql_query("SELECT conff_id FROM conff ORDER BY conff_id DESC LIMIT 1 ;");
	$get_ = mysql_fetch_assoc($lastconff);
	$lastid = $get_['conff_id'];
	$firstconff = mysql_query("SELECT conff_id FROM conff ORDER BY conff_id LIMIT 1 ;");
	$get_ = mysql_fetch_assoc($firstconff);
	$firstid = $get_['conff_id'];
	if($conff_id > $lastid)
		{
			$response["message"] = "No more posts! Please see previous posts!";
			$response["success"] = -1;
		}
	else if($conff_id < $firstid)
		{	$response["message"] = "No more posts behind! Please see next posts!";
			$response["success"] = -2;
		}
	else{
		$next = 1;
											while(true)
											{
												$next_conff = mysql_query("SELECT conff_id FROM conff where conff_id = $conff_id+$next ;");
												if(mysql_num_rows($next_conff) == 1)
													break;
												else
													$next = $next + 1;
											}
											...
											$back = 1;
											while($conff_id - $back > 0)
											{
												$back_conff = mysql_query("SELECT conff_id FROM conff where conff_id = $conff_id-$back ;");
												if(mysql_num_rows($back_conff) == 1)
													break;
												else
													$back = $back + 1;
											}
											$response["next"] = $conff_id+$next;
											$response["back"] =$conff_id-$back;
	}
