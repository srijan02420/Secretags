...
$text = "SELECT * FROM conff where conff_id IN
			(
			SELECT conff_id
					FROM conff_like
					...
					//print_r(mysql_num_rows($get_conff));
						while($row = mysql_fetch_array($get_conff))
						{
							if( $time - $row['activity_time'] < 50 || $_POST['first']==1){
								...
								if(in_multiarray($my_id,$liker,'user_id'))
									$liked = true;
								else 
									$liked = false;
									
								if(in_multiarray($user_id,$commenter,'user_id'))
									$commented = true;
								else 
									$commented = false;
								
								
								$get_who_is = mysql_query("SELECT name,s_pic FROM users where user_id = ".$my_id." ;");
								$get_who_is_info = mysql_fetch_assoc($get_who_is);
								$s_pic = $get_who_is_info['s_pic'];
								$name = $get_who_is_info['name'];
								
								$get_profile = mysql_query("SELECT name FROM users where user_id = ".$user_id." ;");
								$get_who = mysql_fetch_assoc($get_profile);
								$profile_name = $get_who['name'];
								...
								$conff["profile_name"] = $profile_name;
								$conff["profile_id"] = $user_id;
								$conff["profile_commented"] = $commented;
								$conff["tags"] = array();
