				...
				function recentlongPoll(user_id,first)
				{
					if(first==1)
						$( "#loading" ).show();
					
					$.get("http://secretags.com/json/json_get_recent_posts.php", 
					//async = true,
						...
						for(var a=conffs.conff.length-1;a>=0;a--)
							{
								...
								for(var i=0;i<conf.tags.length;i++)
									{tags += '<a href="http://secretags.com/tags/tag/'+conf.tags[i].tag_id+'"><li class="label label-inverse" ><div style = "padding: 4px; font-size:16px;">#'+conf.tags[i].tag+'</div></li></a>'+
								...
								for(i=0;i<conf.comment.length;i++)
									{
										...
										var link = 'http://secretags.com/profile/'+comm.user_id;
										if(comm.gender != 0)
											{com_photo = 'http://secretags.com/images/'+comm.gender+'.jpg';
											link = '#';
											if(comm.gender == 1)
									...
									'<a class="pull-left" href="#" style = "margin-left:10px; margin-top:10px; background:white; border-radius:4px; box-shadow: 0px 0px 10px #000000;"><img src="http://secretags.com/images/'+conf.gender+'.jpg" class="media-object" width = "84px" style = " border-radius:4px;"></a>'+
								'<img src="http://secretags.com/images/type_'+conf.type+'.png" width="30px" height="30px" style="margin-top:79px; margin-left:-32px; position:absolute;">'+
																	...
																	'<div class="row-fluid"  >'+
																		...
																		'<div  style =" background: #DCDCE8; width:100%; padding-top:0px; border-radius : 0px;">'+
																			...
																			'<textarea rows="1"  style = "width:100%; resize:none; border-radius : 0px;  margin : 0 auto; " placeholder="Comment here..." id="commnt'+conf.conff_id+'" class="comment_box"></textarea>'+
																				'<div class="navbar span12 comment_rest" style = "margin : 0 auto;">'+
																					'<div class="navbar-inner">'+
																						'<div class="container row-fluid"  >'+
																							'<div class="span9 offset7">'+
																								'<i class="icon-user"></i>'+
																								'<select id="gender'+conf.conff_id+'" value="Gender" class="span3" placeholder="Comment As">'+
																								 ' <option value="0">Self</option>'+
																								  '<option value="1">Anon Male</option>'+
																								  '<option value="2">Anon Female</option>'+
																								  '<option value="3">Anon Other</option>'+
																								'</select>'+
																								'<button class="btn btn-small" style = "margin-top : -3px; margin-left:10px;" type="button" onClick="add_comment('+conf.conff_id+');return false;">Comment</button>'+
																							'</div>'+
																						'</div>'+
																					'</div>'+
																				'</div>'+
