		...
		function post_conff(){
			...
			if(text.length > 20)
			{
				...
				for(var a=0;a<textarray.length;a++)
					...
					{
						$( "#loading" ).show();
					   $.post("post/post_conff", { gender: $('#conff_gender').val(), user_id: id, conff: $('#conff_text').val().replace(/\n/g, "<br/>"), type: $('#conff_type').val() } )
						.done(function(data) {
							self.location="http://riklr.com/home/recentposts";
						});
					}
					else
						alert("Please add one or more tags by writing '#' just before the tag!");
			}
			else
				alert("Content is not good enough! Please edit and make it better");
			}
		...
		function makeitbig(a,b)
		{
			$('#post_conf'+b).html(unescape(a));
		}
