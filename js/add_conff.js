				...
				function longPoll(user_id,first,all)
				{
					//alert(user_id);
					...
					$.get("http://localhost/Secretags/json/json_get_conff.php", 
					//async = true,
					{ user_id : user_id, first:first,all:all })
					.done(function(data)
					{
						$( "#loading" ).hide();
						...
						for(var a=conffs.conff.length-1;a>=0;a--)
							{
								...
								for(i=0;i<conf.comment.length;i++)
									...
									if(conf.special_type==1)
										special = '<strong style = "font-size:18px; margin-right: 10px;"><a href="http://localhost/Secretags/test/profile/'+conf.special_id+'" style = "color:#002E5C;">'+conf.special_name+'</a> commented on a post</strong></br>';
									else if(conf.special_type==2)
										special = '<strong style = "font-size:18px; margin-right: 10px;"><a href="http://localhost/Secretags/test/profile/'+conf.special_id+'" style = "color:#002E5C;">'+conf.special_name+'</a> liked a post</strong></br>';
										 		varhtml =
												special+
								...
								'<ul class="media inline" style = "margin: 8px; ">'+
									...
									//alert(varhtml);
									//$('#comment_rest'+conf.conff_id).hide();
									//$('#comment'+conf.conff_id).focus('2000',function(){ this.rows=2; $('comment_rest'+conf.conff_id).show(); });
